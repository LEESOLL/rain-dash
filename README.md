...
비 오는 거리를 달려 집까지 가는 게임.<br>단, **세상은 내가 멈추면 천천히 흐르고, 빠르게 움직이면 빠르게 흐른다.**<br>멈추면 비도 번개도 천천히, 달리면 한꺼번에 쏟아진다.

그래서 빠름이 정답이 아니다.<br>달리면 집은 가까워지지만 그만큼 비와 번개를 더 만나고, 멈추면 안전한 대신 한 발자국도 못 나아간다.<br>**결국 속도가 위험과 보상을 한꺼번에 정한다.**
...
**움직이면 빠르게 쌓이고, 멈추면 천천히 쌓인다.**<br>빠르게 벌려면 비·번개가 거센 위험을 감수해야 하고, 안전을 택하면 점수는 더디게 오른다.<br>안전과 고득점을 한꺼번에 가져갈 수는 없다.
...
→ "자주 멈춰 안전하게" vs "쉬지 않고 달려 고득점" — 이 선택이 곧 플레이 스타일이 된다.

---

## 서비스 관점 — 유입·리텐션·확장성

단발 과제가 아니라 **운영하며 리텐션을 키우는 서비스**를 전제로 설계.<br />"한 번 하고 마는 게임"이 아니라 "계속 하고 싶은 게임"이 목표.

### 유입 — 진입 마찰 최소화

익명 인증으로 가입 없이 즉시 플레이.<br>모바일은 텍스트 입력 자체가 마찰(키보드 가림)일 수 있어 닉네임 입력은 자동 생성으로 생략.<br>UX가 불편한 가입 단계는 초기 이탈로 이어지기 쉬운 지점이라, 이를 없애 곧장 플레이로 진입하도록 설계.

### 리텐션 — 돌아올 이유를 단계마다

`진입` → `첫 클리어` → `베스트 갱신(NEW BEST)` → `랭킹 경쟁` → `다음 테마 기대`로 이어지는 흐름.<br>각 단계가 다음을 끌어당기도록 배치.

- **랭킹(누적/테마별)** — 재도전·경쟁 동기.
- **공정성** — 랭킹은 모두가 같은 조건일 때만 의미가 있다. 그래서 웅덩이·아이템·쉘터 위치를 데이터로 고정해 누구나 같은 스테이지를 플레이하게 하고, 비·번개 빈도만 난이도에 따라 바꿔준다. (불공정한 랭킹은 경쟁 동기를 꺾어 리텐션 누수로 이어질 수 있기 때문)

### 확장성 — 콘텐츠를 계속 붙이는 구조

테마마다 스테이지가 묶이는 방식이라, 새 테마를 계속 확장시킬 수 있다.<br>비 오는 거리(4스테이지)는 플레이 가능, 눈 오는 거리·운석 떨어지는 우주는 "Coming Soon"으로 예고해 다음 테마 기대감으로 재방문을 유도한다.
